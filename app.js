...
window.showCloudError = function () {
    ...
    b.innerHTML = `<b>☁️ CLOUD ERROR</b> — Firebase is locked. Data saves locally only.`;
    b.style.cssText = 'background:#ef4444;color:#fff;padding:10px;text-align:center;font-size:13px;position:fixed;top:0;left:0;width:100%;z-index:9999;';
...
const CATEGORIES = ['Health', 'Fitness', 'Learning', 'Work', 'Personal', 'Mindfulness'];
const POM_WORK = 25 * 60;
const POM_BREAK = 5 * 60;
const POM_CIRCUM = 289; // 2πr where r=46
...
const loginScreen = $('loginScreen'), appContainer = $('appContainer');
const noteModal = $('noteModal'), pomodoroModal = $('pomodoroModal');
const templateModal = $('templateModal'), archiveModal = $('archiveModal');
const shareModal = $('shareModal'), themePickerModal = $('themePickerModal');
const shortcutsModal = $('shortcutsModal'), focusModeEl = $('focusMode');
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        const backup = localStorage.getItem(`habitBackup_${user.uid}`);
        if (backup) {
            try { state = JSON.parse(backup); } catch { state = getDefaultState(); }
            migrateState(); renderAll();
        } else {
            state = getDefaultState();
            $('spreadsheet').innerHTML = '<div style="text-align:center;padding:60px;color:#94a3b8;">☁️ Syncing...</div>';
        }
        await loadState();
        renderAll();
...
$('googleSignInBtn').onclick = () => signInWithPopup(auth, new GoogleAuthProvider()).catch(e => alert(e.message));
...
function getDefaultState() {
    return {
        ...
        xp:0, level:1, badges:[], archivedHabits:[], frozenDates:[], streakFreezeUsed:null,
        weeklyChallenge:null, onboardingDone:false, lastLoginDate:null, pomodoroSessions:0
    };
}
...
async function loadState() {
    ...
    try {
        ...
        if (lastSaveTime > ts) return;
        if (snap.exists()) { state = snap.data(); migrateState(); localStorage.setItem(`habitBackup_${currentUser.uid}`, JSON.stringify(state)); }
        else { const b = localStorage.getItem(`habitBackup_${currentUser.uid}`); state = b ? JSON.parse(b) : getDefaultState(); migrateState(); await saveState(); }
    } catch (err) { console.error(err); showCloudError(); const b = localStorage.getItem(`habitBackup_${currentUser.uid}`); state = b ? JSON.parse(b) : getDefaultState(); migrateState(); }
}
async function saveState() {
    ...
    } catch (err) { console.error(err); showCloudError(); }
}

function renderAll() {
    renderSpreadsheet(); updateStats(); updateCharts(); renderHeatmap();
    renderBadges(); renderAvatar(); renderXpBar(); renderDailyQuote();
    renderAnalytics(); generateWeeklyChallenge(); renderWeeklyChallenge();
}

// ─── Undo ─────────────────────────────────────────────────────────────────────
function pushUndo() { undoStack.push(JSON.stringify(state)); if (undoStack.length > 20) undoStack.shift(); }
async function undo() { if (!undoStack.length) { showToast('Nothing to undo!'); return; } state = JSON.parse(undoStack.pop()); await saveState(); showToast('↩️ Undone!'); }
...
$('themeToggleBtn').onclick = () => { renderThemePicker(); themePickerModal.style.display='flex'; setTimeout(()=>themePickerModal.classList.add('visible'),10); };
...
document.addEventListener('keydown', e => {
    ...
    if (e.key==='?') { shortcutsModal.style.display='flex'; setTimeout(()=>shortcutsModal.classList.add('visible'),10); return; }
...
$('shortcutsBtn')?.addEventListener('click', () => { shortcutsModal.style.display='flex'; setTimeout(()=>shortcutsModal.classList.add('visible'),10); });
...
function quickAddHabit(name) {
    ...
    state.habits.push({ id:Date.now().toString()+Math.random(), name:name.trim(), completed:{}, notes:{}, color:colors[state.habits.length%colors.length], frequency:'daily', difficulty:'medium', category:'Personal' });
    $('quickAddInput').value = '';
    saveState();
}
...
function renderSpreadsheet() {
    ...
    const filtered = searchFilter ? state.habits.filter(h => h.name.toLowerCase().includes(searchFilter)) : state.habits;
    const w = window.innerWidth;
    ...
    filtered.forEach((habit, hIdx) => {
        ...
        acts.innerHTML = `<span class="action-icon" data-action="archive" data-id="${habit.id}" title="Archive">📁</span><span class="action-icon" data-action="delete" data-id="${habit.id}" title="Delete">✕</span>`;
        acts.onclick = e => { e.stopPropagation(); const a=e.target.dataset.action; if(a==='delete') deleteHabit(habit.id); if(a==='archive') archiveHabit(habit.id); };

        nameCell.appendChild(diffDot); nameCell.appendChild(nameSpan); nameCell.appendChild(acts);
        ...
        nameCell.ondrop = e => { e.preventDefault(); nameCell.classList.remove('drag-over'); const from=parseInt(e.dataTransfer.getData('text/plain')); if(from!==realIdx){pushUndo();const[m]=state.habits.splice(from,1);state.habits.splice(realIdx,0,m);saveState();} };
        ...
        dates.forEach(d => {
            ...
            let pt; cc.addEventListener('touchstart',()=>{pt=setTimeout(()=>{if(isChecked)openNoteModal(habit,k);},600);}); cc.addEventListener('touchend',()=>clearTimeout(pt));
    ...
    if (selectedCell?.dataset.habitId) container.querySelectorAll('.habit-name').forEach(n=>{if(n.dataset.habitId===selectedCell.dataset.habitId)n.classList.add('selected');});
}
...
function selectCell(cellDiv) {
    ...
    const fs = document.createElement('select'); fs.className='freq-select';
    [['daily','Daily'],['weekdays','Wkdays'],['weekends','Wknds'],['3x','3×/wk'],['2x','2×/wk']].forEach(([v,l])=>{const o=document.createElement('option');o.value=v;o.textContent=l;if(habit.frequency===v)o.selected=true;fs.appendChild(o);});
    fs.onchange=()=>{habit.frequency=fs.value;saveState();};
    fs.onclick=e=>e.stopPropagation();

    const ds = document.createElement('select'); ds.className='freq-select';
    [['easy','⬇Easy'],['medium','→Med'],['hard','⬆Hard']].forEach(([v,l])=>{const o=document.createElement('option');o.value=v;o.textContent=l;if(habit.difficulty===v)o.selected=true;ds.appendChild(o);});
    ds.onchange=()=>{habit.difficulty=ds.value;saveState();};
    ds.onclick=e=>e.stopPropagation();

    const cs = document.createElement('select'); cs.className='freq-select';
    ['Health','Fitness','Learning','Work','Personal','Mindfulness'].forEach(cat=>{const o=document.createElement('option');o.value=cat;o.textContent=cat;if(habit.category===cat)o.selected=true;cs.appendChild(o);});
    cs.onchange=()=>{habit.category=cs.value;saveState();};
    cs.onclick=e=>e.stopPropagation();

    cellDiv.appendChild(inp); cellDiv.appendChild(cp); cellDiv.appendChild(fs); cellDiv.appendChild(ds); cellDiv.appendChild(cs);
    inp.focus();
}
...
function deleteHabit(id) { if(confirm('Delete this habit permanently?')){pushUndo();state.habits=state.habits.filter(h=>h.id!==id);selectedCell=null;saveState();} }
...
$('resetBtn')?.addEventListener('click',()=>{if(confirm('Reset ALL progress?')){pushUndo();state.habits.forEach(h=>{h.completed={};h.notes={};}); state.xp=0;state.level=1;state.badges=[];state.frozenDates=[];state.weeklyChallenge=null; saveState();}});

// ─── Stats ────────────────────────────────────────────────────────────────────
function getStreakCount(s) {
    let streak=0; const temp=new Date(currentDate), frozen=s.frozenDates||[];
    while(true){const k=dateToKey(temp);if(s.habits.some(h=>h.completed[k])||frozen.includes(k)){streak++;temp.setDate(temp.getDate()-1);}else break;}
    return streak;
}
...
function renderAvatar() {
    ...
    if($('avatarLevelBadge')){$('avatarLevelBadge').textContent=`Lv. ${state.level}`;$('avatarLevelBadge').style.cssText=`background:${av.color}33;color:${av.color};`;}
}
...
function updateCharts() {
    ...
    pieChartInstance=new Chart($('pieChart').getContext('2d'),{type:'doughnut',data:{labels:['Done','Missed'],datasets:[{data:[done,Math.max(0,total-done)],backgroundColor:['#7c3aed','rgba(255,255,255,0.06)'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false}});
    if(barChartInstance)barChartInstance.destroy();
    barChartInstance=new Chart($('barChart').getContext('2d'),{type:'bar',data:{labels:hd.map(h=>h.n),datasets:[{label:'Completions',data:hd.map(h=>h.c),backgroundColor:hd.map(h=>h.col),borderRadius:6}]},options:{responsive:true,maintainAspectRatio:false}});
}
...
function generateWeeklyChallenge() {
    ...
    if(state.habits.length>0){const h=state.habits[Math.floor(Math.random()*state.habits.length)];types.push({type:'habit',description:`Complete "${h.name.split(' ')[0]}" every day`,target:7,habitId:h.id,bonusXp:80});}
...
function checkWeeklyChallenge() {
    ...
    if(prog>=target){state.weeklyChallenge.completed=true;state.xp+=state.weeklyChallenge.bonusXp;showMilestoneToast({emoji:'🎯',label:`Weekly Challenge Complete! +${state.weeklyChallenge.bonusXp} XP`});launchConfetti();}
}
function renderWeeklyChallenge() {
    ...
    if($('challengeBarFill')){$('challengeBarFill').style.width=pct+'%';$('challengeBarFill').style.background=ch.completed?'linear-gradient(90deg,#10b981,#22c55e)':undefined;}
...
function renderAnalytics() {
    ...
    for(let i=0;i<30;i++){const d=new Date(currentDate);d.setDate(d.getDate()-i);const k=dateToKey(d),dow=d.getDay(),comps=state.habits.filter(h=>h.completed[k]).length;dt[dow]+=state.habits.length?comps/state.habits.length:0;dc[dow]++;}
    ...
    // Trend
    let l7=0,p7=0;
    for(let i=0;i<7;i++){const d=new Date(currentDate);d.setDate(d.getDate()-i);const k=dateToKey(d);l7+=state.habits.filter(h=>h.completed[k]).length/(state.habits.length||1);}
    for(let i=7;i<14;i++){const d=new Date(currentDate);d.setDate(d.getDate()-i);const k=dateToKey(d);p7+=state.habits.filter(h=>h.completed[k]).length/(state.habits.length||1);}
    l7/=7;p7/=7;
    if($('trendValue')){const diff=p7>0?Math.round((l7-p7)/p7*100):0,up=l7>=p7;$('trendValue').textContent=`${up?'↑':'↓'} ${Math.abs(diff)}% ${up?'🎉 Improving!':'💪 Keep going'}`;$('trendValue').style.color=up?'#10b981':'#f59e0b';}
    // Sparkline
    const wa=[];
    for(let w=3;w>=0;w--){let wt=0;for(let d=0;d<7;d++){const dt=new Date(currentDate);dt.setDate(dt.getDate()-w*7-d);wt+=state.habits.filter(h=>h.completed[dateToKey(dt)]).length/(state.habits.length||1);}wa.push(Math.round(wt/7*100));}
    const sp=$('sparklineContainer');
    if(sp&&wa.length){const sw=130,sh=46,mx=Math.max(...wa,1),pts=wa.map((v,i)=>`${(i/(wa.length-1))*sw},${sh-(v/mx)*sh}`).join(' ');sp.innerHTML=`<svg width="${sw}" height="${sh}" viewBox="0 0 ${sw} ${sh}"><defs><linearGradient id="sg" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stop-color="var(--accent1)"/><stop offset="100%" stop-color="var(--accent2)"/></linearGradient></defs><polyline points="${pts}" fill="none" stroke="url(#sg)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>${wa.map((v,i)=>`<circle cx="${(i/(wa.length-1))*sw}" cy="${sh-(v/mx)*sh}" r="3.5" fill="var(--accent2)"/>`).join('')}</svg><div style="display:flex;justify-content:space-between;font-size:10px;color:var(--sub);margin-top:4px"><span>4w ago</span><span>3w</span><span>2w</span><span>This wk</span></div>`;}
    // Per-habit best streak
    const hs=state.habits.map(h=>{let lng=0,cur=0;for(let i=364;i>=0;i--){const d=new Date(currentDate);d.setDate(d.getDate()-i);if(h.completed[dateToKey(d)]){cur++;lng=Math.max(lng,cur);}else cur=0;}return{name:h.name,lng,col:h.color||'#7c3aed'};}).sort((a,b)=>b.lng-a.lng);
    const sl=$('habitStreakList');
    if(sl){sl.innerHTML='';hs.slice(0,6).forEach(h=>{const r=document.createElement('div');r.className='streak-row';r.innerHTML=`<span class="streak-habit-name">${h.name}</span><span class="streak-badge" style="background:${h.col}22;color:${h.col};border:1px solid ${h.col}44">🔥 ${h.lng}d</span>`;sl.appendChild(r);});if(!hs.length)sl.innerHTML='<span style="color:var(--sub);font-size:13px;">Check off habits to build streaks!</span>';}
    // Monthly Recap
    renderMonthlyRecap();
}
function renderMonthlyRecap() {
    ...
    state.habits.forEach(h=>{let hc=0;for(let d=1;d<=dim;d++){if(h.completed[dateToKey(new Date(yr,mo,d))]){hc++;done++;} total++;}if(hc>bestHabit.c)bestHabit={name:h.name.split(' ')[0],c:hc};});
...
function checkMilestones(){if(!state.badges)state.badges=[];MILESTONES.forEach(m=>{if(!state.badges.includes(m.id)&&m.check(state)){state.badges.push(m.id);showMilestoneToast(m);launchConfetti();}});}
function renderBadges(){const shelf=$('badgesShelf');if(!shelf)return;if(!state.badges)state.badges=[];shelf.innerHTML='';MILESTONES.forEach(m=>{const earned=state.badges.includes(m.id),card=document.createElement('div');card.className=`badge-card${earned?' earned':' locked'}`;card.title=earned?'Achieved!':'Locked';card.innerHTML=`<span class="badge-emoji">${earned?m.emoji:'🔒'}</span><span class="badge-label">${m.label}</span>`;shelf.appendChild(card);});}
function showMilestoneToast(m){const t=$('milestoneToast');t.innerHTML=`${m.emoji} <b>Achievement!</b> — ${m.label}`;t.style.display='flex';setTimeout(()=>t.classList.add('visible'),50);setTimeout(()=>{t.classList.remove('visible');setTimeout(()=>t.style.display='none',500);},3500);}

// ─── Confetti ─────────────────────────────────────────────────────────────────
function launchConfetti(){const c=$('confettiCanvas'),ctx=c.getContext('2d');c.width=window.innerWidth;c.height=window.innerHeight;const cols=['#4f46e5','#ec4899','#f59e0b','#10b981','#0ea5e9'],ps=Array.from({length:100},()=>({x:Math.random()*c.width,y:Math.random()*-c.height,r:Math.random()*7+2,col:cols[Math.floor(Math.random()*cols.length)],speed:Math.random()*4+2,ta:0}));let fr=0;function draw(){ctx.clearRect(0,0,c.width,c.height);ps.forEach(p=>{p.ta+=0.1;p.y+=p.speed;p.x+=Math.sin(p.ta)*2;ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fillStyle=p.col;ctx.globalAlpha=Math.max(0,1-fr/120);ctx.fill();});ctx.globalAlpha=1;fr++;if(fr<140)requestAnimationFrame(draw);else ctx.clearRect(0,0,c.width,c.height);}draw();}

// ─── Daily Quote ──────────────────────────────────────────────────────────────
const QUOTES=[{t:'We are what we repeatedly do. Excellence is not an act but a habit.',a:'Aristotle'},{t:'Success is the sum of small efforts, repeated day in and day out.',a:'Robert Collier'},{t:'Motivation is what gets you started. Habit is what keeps you going.',a:'Jim Ryun'},{t:'The secret of getting ahead is getting started.',a:'Mark Twain'},{t:'You are what you do, not what you say you\'ll do.',a:'C.G. Jung'},{t:'Small daily improvements over time lead to stunning results.',a:'Robin Sharma'},{t:'It\'s not what we do once in a while that shapes our lives. It\'s what we do consistently.',a:'Tony Robbins'},{t:'Discipline is the bridge between goals and accomplishment.',a:'Jim Rohn'},{t:'The chains of habit are too light to be felt until they are too heavy to be broken.',a:'Warren Buffett'},{t:'A journey of a thousand miles begins with a single step.',a:'Lao Tzu'},{t:'Don\'t watch the clock; do what it does. Keep going.',a:'Sam Levenson'},{t:'Action is the foundational key to all success.',a:'Pablo Picasso'},{t:'The future depends on what you do today.',a:'Mahatma Gandhi'},{t:'Either you run the day, or the day runs you.',a:'Jim Rohn'},{t:'One day or day one. You decide.',a:'Unknown'},{t:'A little progress each day adds up to big results.',a:'Unknown'},{t:'Fall seven times, stand up eight.',a:'Japanese Proverb'},{t:'Your habits will determine your future.',a:'Jack Canfield'},{t:'Success doesn\'t come from what you do occasionally. It comes from what you do consistently.',a:'Marie Forleo'},{t:'We first make our habits, then our habits make us.',a:'John Dryden'},{t:'Winning is a habit. Unfortunately, so is losing.',a:'Vince Lombardi'},{t:'Do something today that your future self will thank you for.',a:'Unknown'},{t:'Strive for progress, not perfection.',a:'Unknown'},{t:'The best time to start was yesterday. The next best time is now.',a:'Unknown'},{t:'Focus on progress, not perfection.',a:'Unknown'},{t:'All our life is but a mass of habits.',a:'William James'},{t:'Good habits formed at youth make all the difference.',a:'Aristotle'},{t:'You don\'t rise to the level of your goals. You fall to the level of your systems.',a:'James Clear'},{t:'Every action you take is a vote for the type of person you wish to become.',a:'James Clear'},{t:'The quality of your life depends on the quality of your habits.',a:'Unknown'}];
function renderDailyQuote(){const el=$('quoteText');if(!el)return;const doy=Math.floor((currentDate-new Date(currentDate.getFullYear(),0,0))/86400000),q=QUOTES[doy%QUOTES.length];el.innerHTML=`"${q.t}" <span style="opacity:.55;font-size:12px">— ${q.a}</span>`;}
...
function pomodoroTick(){
    if(pomState.timeLeft>0){pomState.timeLeft--;renderPomodoroDisplay();}
        ...
        renderPomodoroDisplay();if($('pomStartBtn'))$('pomStartBtn').textContent='▶ Start';
        saveState();
    }
}
...
function showPomodoroModal(){
    ...
    pomodoroModal.style.display='flex';setTimeout(()=>pomodoroModal.classList.add('visible'),10);
}
...
function renderFocusMode(){
    ...
    if($('focusDoneCount'))$('focusDoneCount').textContent=done;if($('focusTotalCount'))$('focusTotalCount').textContent=total;
...
function showTemplateModal(){
    ...
    templateModal.style.display='flex';setTimeout(()=>templateModal.classList.add('visible'),10);
}
function applyTemplate(key){const pack=HABIT_TEMPLATES[key];if(!pack)return;pushUndo();pack.habits.forEach(h=>state.habits.push({id:Date.now().toString()+Math.random(),name:h.name,completed:{},notes:{},color:h.color,frequency:'daily',difficulty:h.difficulty,category:h.category}));showToast(`✅ ${pack.name} added!`);saveState();}
...
function renderArchiveModal(){
    ...
    archived.forEach(h=>{const row=document.createElement('div');row.className='archive-row';row.style.borderLeftColor=h.color||'#7c3aed';row.innerHTML=`<span class="archive-name">${h.name}</span><button class="btn" style="font-size:12px;padding:6px 14px" data-id="${h.id}">↩️ Restore</button>`;row.querySelector('button').onclick=()=>restoreHabit(h.id);list.appendChild(row);});
}
$('archiveBtn')?.addEventListener('click',()=>{renderArchiveModal();archiveModal.style.display='flex';setTimeout(()=>archiveModal.classList.add('visible'),10);});
$('closeArchiveBtn')?.addEventListener('click',()=>closeModal('archiveModal'));

// ─── Share Card ───────────────────────────────────────────────────────────────
function renderShareCard(){
    const canvas=$('shareCanvas');if(!canvas)return;
    canvas.width=600;canvas.height=340;const ctx=canvas.getContext('2d');
...
$('shareBtn')?.addEventListener('click',()=>{renderShareCard();shareModal.style.display='flex';setTimeout(()=>shareModal.classList.add('visible'),10);});
...
// ─── Onboarding ───────────────────────────────────────────────────────────────
function showOnboarding(){if(!onboardingOverlay)return;['ob-step-1','ob-step-2','ob-step-3'].forEach((id,i)=>{const el=$(id);if(el)el.classList.toggle('active',i===0);});onboardingOverlay.style.display='flex';setTimeout(()=>onboardingOverlay.classList.add('visible'),10);}
function closeOnboarding(){onboardingOverlay.classList.remove('visible');setTimeout(()=>onboardingOverlay.style.display='none',300);state.onboardingDone=true;saveState();}
...
// ─── Note Modal ───────────────────────────────────────────────────────────────
function openNoteModal(habit,dateKey){selectedNoteTarget={habit,dateKey};const d=new Date(dateKey);$('noteModalLabel').textContent=`${habit.name} — ${d.toDateString()}`;$('noteInput').value=habit.notes?.[dateKey]||'';noteModal.style.display='flex';setTimeout(()=>noteModal.classList.add('visible'),10);$('noteInput').focus();}
...
$('saveNoteBtn')?.addEventListener('click',()=>{if(!selectedNoteTarget)return;const{habit,dateKey}=selectedNoteTarget;if(!habit.notes)habit.notes={};const v=$('noteInput').value.trim();if(v)habit.notes[dateKey]=v;else delete habit.notes[dateKey];closeNoteModal();saveState();});
$('cancelNoteBtn')?.addEventListener('click',closeNoteModal);
noteModal?.addEventListener('click',e=>{if(e.target===noteModal)closeNoteModal();});

// ─── Audio ────────────────────────────────────────────────────────────────────
function playCheckSound(){try{const ac=new(window.AudioContext||window.webkitAudioContext)(),osc=ac.createOscillator(),g=ac.createGain();osc.connect(g);g.connect(ac.destination);osc.frequency.setValueAtTime(880,ac.currentTime);osc.frequency.exponentialRampToValueAtTime(660,ac.currentTime+0.1);g.gain.setValueAtTime(0.1,ac.currentTime);g.gain.exponentialRampToValueAtTime(0.001,ac.currentTime+0.25);osc.start(ac.currentTime);osc.stop(ac.currentTime+0.25);}catch{}}

// ─── Notifications ────────────────────────────────────────────────────────────
function scheduleReminder(){const t=new Date();t.setHours(20,0,0,0);if(new Date()>t)t.setDate(t.getDate()+1);setTimeout(()=>{const done=state.habits.length>0&&state.habits.every(h=>h.completed[todayKey]);if(!done&&Notification.permission==='granted')new Notification('🔥 Habit Reminder',{body:"Don't forget your habits! Keep that streak alive! 💪"});},t-new Date());}
$('notifBtn')?.addEventListener('click',()=>{'Notification'in window&&Notification.requestPermission().then(p=>{if(p==='granted'){$('notifBtn').textContent='🔔 On';showToast('🔔 Reminders enabled!');}else showToast('Allow notifications in browser settings.');});});

// ─── CSV Export / Import ──────────────────────────────────────────────────────
$('downloadCsvBtn')?.addEventListener('click',()=>{let csv='Habit,Difficulty,Category,'+dates.map(d=>d.toLocaleDateString()).join(',')+'\n';state.habits.forEach(h=>{csv+=`"${h.name}",${h.difficulty},${h.category},`+dates.map(d=>h.completed[dateToKey(d)]?'✓':'').join(',')+'\n';});const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));a.download=`habits-${todayKey}.csv`;a.click();});
$('importCsvBtn')?.addEventListener('click',()=>$('csvFileInput').click());
$('csvFileInput')?.addEventListener('change',e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=ev=>{const lines=ev.target.result.split('\n').filter(l=>l.trim());const colors=['#7c3aed','#0ea5e9','#10b981','#f59e0b','#ec4899'];let added=0;lines.forEach((l,i)=>{if(i===0&&l.toLowerCase().includes('habit'))return;const name=l.split(',')[0].replace(/"/g,'').trim();if(name){pushUndo();state.habits.push({id:Date.now().toString()+Math.random(),name,completed:{},notes:{},color:colors[added%colors.length],frequency:'daily',difficulty:'medium',category:'Personal'});added++;}});if(added>0){showToast(`📂 Imported ${added} habit${added>1?'s':''}!`);saveState();}else showToast('No habits found in CSV.');$('csvFileInput').value='';};r.readAsText(f);});
$('printReportBtn')?.addEventListener('click',()=>window.print());

// ─── Shared Utilities ─────────────────────────────────────────────────────────
function closeModal(id){const m=$(id);if(!m)return;m.classList.remove('visible');setTimeout(()=>m.style.display='none',300);}
[templateModal,archiveModal,shareModal,themePickerModal,shortcutsModal,pomodoroModal].forEach(m=>{m?.addEventListener('click',e=>{if(e.target===m){m.classList.remove('visible');setTimeout(()=>m.style.display='none',300);}});});
